...
// #include <eosio/chain/new_types.hpp>
...
class wallet_api
{
   public:
      ...
      void    lock();
      /**
       * Get the WIF private key corresponding to a password.  The
       * private key must already be in the wallet.
       */
      // private_key_type get_private_key(const string& password);
      string get_private_key( string password);
      ...
      pair<public_key_type, private_key_type> list_keys();
...
FC_REFLECT( eosio::wallet::plain_keys, (checksum)(key) )
