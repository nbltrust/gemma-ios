...
open class AttributedLabelView: UIView {
    ...
    open var onClick: ((AttributedLabelView, Detection) -> Void)?
    ...
    open override func layoutSubviews() {
        ...
        if let (text, string) = state.attributedTextAndString {
            ...
            highlightableDetections.forEach { detection in
                ...
                layoutManager.enumerateEnclosingRects(forGlyphRange: nsrange, withinSelectedGlyphRange: NSRange(location: NSNotFound, length: 0), in: textContainer, using: { (rect, _) in
                    var finalRect = rect
...
extension NSAttributedString {
    ...
    fileprivate func withInherited(font: UIFont, textAlignment: NSTextAlignment) -> NSAttributedString {
        ...
        result.beginEditing()
        enumerateAttributes(in: NSRange(location: 0, length: length), options: .longestEffectiveRangeNotRequired, using: { (attributes, range, _) in
